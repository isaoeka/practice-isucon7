...
func queryIdToUsers(messages []*Message) (map[int64]*User, error) {
	// "message":"empty slice passed to 'in' query" の対応
	if len(messages) <= 0 {
		return nil, nil
	}
	...
	var users []*User
	if err := db.Select(&users, q, vs...); err != nil {
		return nil, err
	}
...
// なんとなく残しておく
func queryImages() ([]Image, error) {
	res := []Image{}
	err := db.Select(&res, "SELECT * FROM image")
	return res, err
}
...
func jsonifyMessage(m *Message, user *User) (*JsonMessage, error) {
...
func getMessage(c echo.Context) error {
	...
	idToUser, err := queryIdToUsers(messages)
	if err != nil {
		return err
	}

	response := make([]*JsonMessage, 0)
	for i := len(messages) - 1; i >= 0; i-- {
		m := messages[i]

		user := idToUser[m.UserID]
		jsonMessage, err := jsonifyMessage(m, user)
		if err != nil {
			return err
		}

		response = append(response, jsonMessage)
	}
...
func getHistory(c echo.Context) error {
	...
	mjson := make([]*JsonMessage, 0)
	for i := len(messages) - 1; i >= 0; i-- {
		user := idToUser[messages[i].UserID]
...
func postProfile(c echo.Context) error {
	...
	if avatarName != "" && len(avatarData) > 0 {
		// ファイル書き出し
		if err := writeImage(avatarName, avatarData); err != nil {
			return err
		}
